...
export const GROWTH_COORDINATION_RULES: string[] = [
  ...
  "sem-specialist (Google Ads) es un especialista mas de esta pasada, con exactamente las mismas reglas que los demas: si su `status` es `executed`, su salida es una lectura REAL de la cuenta hecha en esta misma pasada y debes tratarla como tal (citando `dept-sem-*`); si NO lo es, no infieras NADA sobre Google Ads -- ni gasto, ni CPC, ni impresiones, ni campanas activas -- y declaralo en `dependencies[]` como `missing`. Las incognitas que sem-specialist declare en su propia salida (`dept-sem-unknowns`) son datos que NO existen en la cuenta o no se pudieron leer: no las conviertas en conclusiones.",
