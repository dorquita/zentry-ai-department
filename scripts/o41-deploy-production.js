...
// PRODUCTION ONLY. No staging config object exists in this script on purpose.
...
// ---- CTA ficha/categoria (final O40 version) -> goes into existing snippet id 17 ----
const CTA_FICHA_CATEGORIA_CODE = `add_action('woocommerce_after_add_to_cart_button', function () {
    global $product;
...
// ---- CSS compartido (final O40.3 version) ----
...
// ---- WhatsApp flotante (final O40.3 version) ----
...
// ---- CTA landings (final O40 version, unchanged by O40.3) ----
...
// ---- Tracking JS (final O40 version, unchanged by O40.3) ----
...
// ---- Sticky bar (final O40.3 version with scroll-reveal) ----
...
// ---- Hero refinement (final O40.3 version, same Kadence block classes confirmed identical on production) ----
...
async function main() {
  const results = {};
  const toCreate = [
    ["css", "O40 - CSS CTA presupuesto + WhatsApp - PRODUCCION", "Estilos aislados para el CTA de presupuesto, el boton flotante de WhatsApp y la barra sticky inferior. Replicado desde staging (O40/O40.1/O40.3), version final. Fase O41.", CSS_CODE],
    ["wa", "O40 - WhatsApp flotante - PRODUCCION", "Boton flotante de WhatsApp, numero real 34630158767 (ya usado en la pagina Contacto real de este mismo sitio), texto desktop 'Te respondemos rapido' (O40.3), mensaje dinamico segun producto, se oculta mientras el banner de cookies Complianz esta visible. Replicado desde staging. Fase O41.", WHATSAPP_CODE],
    ["landing", "O40 - CTA presupuesto personalizado (landings) - PRODUCCION", "CTA de presupuesto personalizado anadido al final del contenido de las 7 landings comerciales (taquillas-melamina, taquillas-fenolicas, taquillas-fenolicas-perfileria, taquillas-para-colegios, taquillas-para-empresas, cerraduras-para-taquillas, digitalizacion-taquillas). Replicado desde staging. Fase O41.", LANDING_CTA_CODE],
    ["tracking", "O40 - JS tracking CTA + WhatsApp + sticky - PRODUCCION", "Listener delegado de clicks sobre [data-zentry-event] (click_presupuesto_personalizado, click_whatsapp_floating, sticky_presupuesto_click), envia evento via gtag() si existe o dataLayer.push como fallback. No activa conversiones ni campanas de Ads. Replicado desde staging. Fase O41.", TRACKING_CODE],
    ["bar", "O40.1/O40.3 - Sticky CTA presupuesto inferior - PRODUCCION", "Barra fija inferior azul/amarilla tipo Taquiblock, oculta en carrito/checkout/contacto y mientras el banner de cookies esta abierto, con aparicion diferida por scroll (350px en home, 120px en el resto) para no saturar la primera pantalla. Replicado desde staging, version final O40.3. Fase O41.", STICKY_BAR_CODE],
    ["hero", "O40.3 - Hero refinamiento visual - PRODUCCION", "Reduce el H1 del hero de home ~13% en desktop (64px->56px) y ajusta el espaciado hacia el subtitulo, solo en is_front_page(), usando las clases especificas del bloque Kadence (kt-adv-heading1878_hero-a3/a4) confirmadas identicas a staging. Replicado desde staging. Fase O41.", HERO_CODE],
  ];

  for (const [key, name, desc, code] of toCreate) {
    const created = await createSnippet(name, desc, code);
    console.log(key, "created:", JSON.stringify({ id: created.id, code_error: created.code_error }));
    results[key + "_created"] = created;
    if (created.code_error !== null || !created.id) {
      console.log("\nABORTING at", key, "- code_error present. Nothing activated, snippet 17 not touched.");
      fs.writeFileSync("/opt/zentry-ai-department/reports/o41-deploy-result.json", JSON.stringify({ results, ok: false, stoppedAt: key }, null, 2));
      return;
    }
  }

  for (const [key] of toCreate) {
    const id = results[key + "_created"].id;
    const act = await activateSnippet(id);
    console.log(key, "activated:", JSON.stringify({ id: act.id, active: act.active, code_error: act.code_error }));
    results[key + "_activated"] = act;
    if (act.code_error !== null || act.active !== true) {
      console.log("\nABORTING - activation of", key, "was not clean. Snippet 17 not touched.");
      fs.writeFileSync("/opt/zentry-ai-department/reports/o41-deploy-result.json", JSON.stringify({ results, ok: false, stoppedAt: key + "_activation" }, null, 2));
      return;
    }
  }

  const cta17 = await updateSnippet(17, { code: CTA_FICHA_CATEGORIA_CODE });
  console.log("\nsnippet 17 (CTA ficha/categoria) updated:", JSON.stringify({ id: cta17.id, active: cta17.active, code_error: cta17.code_error }));
  results.cta17_updated = cta17;

  fs.writeFileSync("/opt/zentry-ai-department/reports/o41-deploy-result.json", JSON.stringify({ results, ok: cta17.code_error === null }, null, 2));
  console.log("\n=== DONE ===");
}
