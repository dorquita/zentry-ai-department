/**
 * Genera un refresh token OAuth2 de SOLO LECTURA para Google Ads.
 *
 * Scope solicitado (unico): https://www.googleapis.com/auth/adwords
 * (la API de Google Ads no tiene un scope mas estrecho que "adwords";
 * el modo solo-lectura lo garantiza este proyecto en
 * src/adapters/google-ads.ts, que unicamente llama a `googleAds:search`
 * y no importa ningun endpoint `:mutate` — ver docs/google-ads-readonly.md).
 *
 * Mismo patron y garantias que `npm run auth:gsc`
 * (scripts/get-gsc-refresh-token.ts):
 *   1. Reutiliza GOOGLE_ADS_OAUTH_CLIENT_ID/SECRET si ya estan en .env
 *      (no los vuelve a pedir ni los imprime). Si no estan, los pide con
 *      entrada enmascarada.
 *   2. Genera una URL de autorizacion (access_type=offline, prompt=consent)
 *      para abrir en cualquier navegador — no hace falta que sea en este
 *      servidor. TU inicias sesion y confirmas el consentimiento.
 ...
 *
 * Que NO hace:
 *   - No activa campanas, no cambia presupuesto, no crea keywords ni
 *     anuncios, no toca conversiones — este script solo obtiene un token
 *     de autenticacion, no ejecuta ninguna llamada a la API de Ads mas
 *     alla del intercambio OAuth2 estandar (token endpoint de Google).
 *   - No toca GA4, GTM, WordPress, n8n ni qdrant.
...
/**
 * Backup de .env ANTES de escribir nada, mismo formato de nombre que se
 * ha usado a mano en fases anteriores (.env.bak.<YYYYMMDDHHmmss>).
 * Permisos 600 preservados en la copia.
 */
...
async function main() {
  console.log("=== Refresh token OAuth2 de solo lectura para Google Ads ===");
  console.log(`Scope solicitado (unico): ${SCOPE}`);
  console.log(
    "Este script SOLO obtiene un token de autenticacion — no activa campanas, no cambia"
  );
  console.log(
    "presupuesto, no crea keywords/anuncios, no toca conversiones. Ver docs/google-ads-readonly.md."
  );
  console.log("");

  const clientId = await resolveCredential("GOOGLE_ADS_OAUTH_CLIENT_ID", "Client ID");
  const clientSecret = await resolveCredential("GOOGLE_ADS_OAUTH_CLIENT_SECRET", "Client Secret");
  const redirectUri = process.env.GOOGLE_ADS_OAUTH_REDIRECT_URI ?? DEFAULT_REDIRECT_URI;
  ...
  console.log("   e inicia sesion con la cuenta de Google que tiene acceso a la cuenta de Google Ads");
  console.log("   (customer 8369126564, via la MCC 3080169697). Veras una pantalla de consentimiento");
  console.log("   pidiendo acceso a Google Ads:");
  console.log("");
  console.log(authUrl);
  console.log("");
  console.log(
  ...
  console.log("");
  console.log(
    "Intercambiando el authorization code por tokens (llamada estandar de OAuth2, sin acciones sobre Ads)..."
  );
  ...
  console.log("");
  console.log("OK: refresh token obtenido (solo lectura, scope adwords).");
  ...
  const confirm = await plainPrompt(
    `Guardar automaticamente en ${ENV_PATH}? Antes de escribir se creara un backup del .env actual.\n` +
      "Se anadiran/actualizaran SOLO estas claves (GOOGLE_ADS_OAUTH_CLIENT_ID, GOOGLE_ADS_OAUTH_CLIENT_SECRET,\n" +
      'GOOGLE_ADS_OAUTH_REFRESH_TOKEN), el resto del fichero no se toca. Escribe "si" para confirmar: '
  );
  ...
  if (confirm.trim().toLowerCase() === "si" || confirm.trim().toLowerCase() === "s") {
    ...
    upsertEnvFile(ENV_PATH, {
      ...
      GOOGLE_ADS_OAUTH_REFRESH_TOKEN: tokens.refresh_token,
    });
    console.log("");
    console.log(`Guardado en ${ENV_PATH} (permisos 600). El valor completo no se ha impreso en ningun momento.`);
    console.log(
      "Revisa que GOOGLE_ADS_DEVELOPER_TOKEN, GOOGLE_ADS_CUSTOMER_ID y GOOGLE_ADS_LOGIN_CUSTOMER_ID"
    );
