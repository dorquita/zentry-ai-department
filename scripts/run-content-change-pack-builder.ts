import * as dotenv from "dotenv";
dotenv.config();

import { runContentChangePackBuilder } from "../src/agents/content-change-pack-builder";

runContentChangePackBuilder()
  .then((result) => {
    console.log(`\nContent Change Pack Builder — departmentRunId: ${result.departmentRunId}`);
    console.log(`Nuevos: ${result.newChangePacks.length}`);
    console.log(`Ya existentes: ${result.alreadyExisting.length}`);
    console.log(`\nInforme: ${result.reportPath}`);
    process.exit(0);
  })
  .catch((err) => {
    console.error("Content Change Pack Builder fallo:", err instanceof Error ? err.message : err);
    process.exit(1);
  });
